...
include($_SERVER['DOCUMENT_ROOT'].'/api/core/database.php');
	...
	function read($db,$data){
		if(isset($data['user'])){//exact id
			if(!$db->prepare("SELECT * FROM users WHERE user_id=? AND room_id=?;")){
				return false;
			}
			if(!$db->execute(array($data['user'],$data['room']))){
				return false;
			}
			return $db->fetch_rows_assoc();

		}else{//all rooms
			if(!$db->prepare("SELECT * FROM users WHERE room_id=?;")){
				return false;
			}
			if(!$db->execute(array($data['room']))){
				return false;
			}
			return $db->fetch_rows_assoc();
